...
export const copyS3Folder = async (frameWork: string, userId: string, replId:string) => {
    const bucket = process.env.S3_BUCKET
    // const replId = getRandomSlug()

    const sourcePrefix = `base/${frameWork}`
    const destinationPrefix = `user/${userId}/${frameWork}-${replId}`
